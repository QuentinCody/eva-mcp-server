...
export function registerQueryData(server: McpServer, env?: QueryEnv): void {
    const handler = createQueryDataHandler("EVA_DATA_DO", "eva");

    server.registerTool(
        "eva_query_data",
        {
            title: "Query Staged EVA Data",
            description:
                "Query staged European Variation Archive data using SQL. Use this when responses are too large and have been staged with a data_access_id.",
            inputSchema: {
